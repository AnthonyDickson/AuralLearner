package cosc345.app.lib;//will include hertz frequincy to pitch conversion
...
public class Note {
    ...
    /*
     * cosc345.app.lib.Note is represented by a char
     */
    ...
    public Note(Note noteInput, int interval) {
        char lowerNote = noteInput.note.charAt(0);
    ...
    public void detectNote() {
        ...
        for (int i = 0; i < pitch.length - 1; i++) {
            ...
            if (hertzAVG > pitch[i] && hertzAVG <pitch[i] + (difference) / 2){
                offsetFromPitch = hertzAVG - pitch[i];
                difference = (pitch[i + 1] - pitch[i]);
                pitchMatched = pitch[i];
                break;
            } else if (hertzAVG > pitch[i] * 2 && hertzAVG < pitch[i] * 2 + (differce * 2) / 2) {
